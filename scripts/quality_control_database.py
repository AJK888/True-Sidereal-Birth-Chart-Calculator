...
def check_record(record: FamousPerson, report: QualityControlReport):
    ...
    # 4. unknown_time consistency - CRITICAL: Ascendants should NOT be populated when time is unknown
    if record.unknown_time:
        # If unknown_time is True, rising signs should be None
        if record.rising_sign_sidereal is not None:
            report.add_issue(record.id, record.name or "N/A", "ASCENDANT_WITH_UNKNOWN_TIME", 
                           f"unknown_time=True but rising_sign_sidereal={record.rising_sign_sidereal} (should be None)")
            has_issues = True
        if record.rising_sign_tropical is not None:
            report.add_issue(record.id, record.name or "N/A", "ASCENDANT_WITH_UNKNOWN_TIME", 
                           f"unknown_time=True but rising_sign_tropical={record.rising_sign_tropical} (should be None)")
            has_issues = True
    else:
    ...
    for sign_field, sign_value in [
        ...
        ("rising_sign_sidereal", record.rising_sign_sidereal),
        ("rising_sign_tropical", record.rising_sign_tropical),
    ]:
        if sign_value:
    ...
    if record.chinese_zodiac_element and (record.chinese_zodiac_element.upper() == "N/A" or record.chinese_zodiac_element == "N/A"):
        report.add_issue(record.id, record.name or "N/A", "N_A_VALUE", 
                       f"chinese_zodiac_element is 'N/A' (should be calculated)")
        has_issues = True
    elif record.chinese_zodiac_element and record.chinese_zodiac_element not in VALID_CHINESE_ELEMENTS:
        report.add_issue(record.id, record.name or "N/A", "INVALID_CHINESE_ELEMENT", 
                       f"chinese_zodiac_element has invalid value: {record.chinese_zodiac_element}")
        has_issues = True
    ...
    if not record.chinese_zodiac_animal:
        ...
        has_issues = True
    if not record.chinese_zodiac_element:
        report.add_issue(record.id, record.name or "N/A", "MISSING_CHINESE_ZODIAC", 
                       "Missing chinese_zodiac_element")
        has_issues = True
