...
def fix_ascendants_for_unknown_time(db):
    ...
    for record in all_unknown_time_records:
        ...
        if record.rising_sign_sidereal is not None or record.rising_sign_tropical is not None:
            record.rising_sign_sidereal = None
            record.rising_sign_tropical = None
            fixed_in_columns += 1
            needs_fix = True
        
        # Check chart_data_json for ascendant data
    ...
    if fixed_count > 0:
        ...
        print(f"  - Fixed ascendant in database columns: {fixed_in_columns}")
...
def populate_missing_numerology(db):
    ...
    for record in all_records:
        ...
        needs_chinese = ((not record.chinese_zodiac_animal or record.chinese_zodiac_animal.upper() == "N/A") or
                        (not record.chinese_zodiac_element or record.chinese_zodiac_element.upper() == "N/A"))
            ...
            try:
                ...
                if needs_chinese:
                    ...
                    if chinese_zodiac_str and chinese_zodiac_str != "N/A":
                        ...
                        if len(parts) >= 2:
                            if needs_chinese:
                                if not record.chinese_zodiac_element or record.chinese_zodiac_element.upper() == "N/A":
                                    record.chinese_zodiac_element = parts[0]
                                    updated = True
        ...
        if needs_recalculation or needs_life_path or needs_day_number or needs_chinese:
            try:
                ...
                if needs_chinese:
                    ...
                    if chinese_result:
                        if not record.chinese_zodiac_element or record.chinese_zodiac_element.upper() == "N/A":
                            record.chinese_zodiac_element = chinese_result.get("element")
                            updated = True
                        if not record.chinese_zodiac_animal or record.chinese_zodiac_animal.upper() == "N/A":
