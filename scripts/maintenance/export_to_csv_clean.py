"""Export all famous people data to CSV file - only current model columns"""
...
def export_to_csv(output_file=None):
    """Export all famous people records to CSV with only current model columns."""
    ...
    # Define CSV columns (only what's in the current model)
    fieldnames = [
        'id',
        'name',
        'wikipedia_url',
        ...
        'updated_at',
        'has_chart_data_json',
        'has_planetary_placements_json',
        'has_top_aspects_json'
    ]
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for record in records:
            try:
                ...
                print(f"Error exporting record {record.id} ({record.name}): {e}")
                continue
    ...
    print(f"\n✓ Successfully exported {exported_count} records to {output_file}")
