"""Test the fixed get_infobox_data function."""
import sys
import os
...
for name in test_cases:
    ...
    if result and result.get('birth_date') and result.get('birth_location'):
        ...
        if birth_date.get('year') and birth_date.get('month') and birth_date.get('day'):
            ...
            results.append((name, False, "Incomplete date"))
    else:
        print(f"  ❌ Would NOT be added (missing date or location)")
        results.append((name, False, "Missing data"))
...
with open('test_results.json', 'w') as f:
    json.dump({name: result for name, _, _ in results}, f, indent=2, default=str)
